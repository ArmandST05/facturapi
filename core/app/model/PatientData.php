...
class PatientData {
	...
	public static function getByIdForInvoice($patientId)
	{
		// Escapar el parámetro para prevenir inyección SQL
		$patientId = mysqli_real_escape_string(Database::getCon(), $patientId);
		$sql = "SELECT name, email, street, number AS exterior, colony AS neighborhood FROM " . self::$tablename . " WHERE id = '$patientId'";
		$query = Executor::doit($sql);
		return Model::one($query[0], new PatientData()); // Ajusta si es necesario
	}
