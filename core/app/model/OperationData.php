...
class OperationData
{
	...
	{
		$sql = "SELECT s.*,DATE_FORMAT(s.created_at,'%d/%m/%Y') AS date_format,DATE_FORMAT(s.created_at,'%H:%i:%s') AS hour_format FROM " . self::$tablename . " s WHERE s.id=$id";
		$query = Executor::doit($sql);
		return Model::one($query[0], new OperationData());
	}
	...
	public static function getAllMedicSalesByProductDates($productId, $startDate,$endDate)
	{
		$startDateTime = $startDate . " 00:00:00";
		$endDateTime = $endDate . " 23:59:59";
...
{
    // Escapar el parámetro para prevenir inyección SQL
    $operationId = mysqli_real_escape_string(Database::getCon(), $operationId);
    $sql = "SELECT operation_id, product_id, quantity, price FROM " . self::$tablename . " WHERE operation_id = '$operationId'";
    $query = Executor::doit($sql);
    return Model::many($query[0], new OperationDetailData()); // Ajusta si es necesario
}
