...
AND cat.is_active = 1 AND con.is_active = 1";
...
if( !empty($requestData['search']['value']) ) {
	...
	$sql.=" WHERE con.expense_category_id = cat.id AND con.type_id='2' AND cat.is_active = 1 AND con.is_active = 1 ";
	...
	$sql = "SELECT con.id,con.name,cat.name category_name";
	$sql.=" FROM products con, expense_categories cat";
	$sql.=" WHERE con.expense_category_id=cat.id AND cat.is_active = 1 AND con.is_active = 1";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
    ...
    $nestedData[] =  '<td >
			   ...
			   <a href="index.php?action=expense-concepts/update-status&id='.$row["id"].'&isActive=0" class="btn btn-danger btn-xs" onclick="return confirmDelete()"><i class="fas fa-trash"></i></a>	     
			   </td>';	
