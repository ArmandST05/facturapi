...
(SELECT COUNT(r.id) FROM reservations r WHERE r.status_id = 3 AND r.patient_id = p.id) AS total_absences
FROM patients p, patient_categories pc WHERE p.category_id = pc.id ";
if ($absenceType == "absences"){
	$sql .= " AND (SELECT COUNT(r.id) FROM reservations r WHERE r.status_id = 3 AND r.patient_id = p.id) >= 1 ";
}
...
if( !empty($requestData['search']['value']) ) {
	...
	$sql = "SELECT p.*,pc.name as patient_category_name, pc.color as patient_category_color, ";
	$sql.= " (SELECT COUNT(r.id) FROM reservations r WHERE r.status_id = 3 AND r.patient_id = p.id) AS total_absences ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = "<td style=background-color:".$row['patient_category_color']."; >".$row["id"]."</td>";
    $nestedData[] = "<span style='background-color:".$absenceColor."'>".$row["name"]."</span>";
